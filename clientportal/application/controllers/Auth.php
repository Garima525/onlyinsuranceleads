...
class Auth extends CI_Controller {
    ...
    public function index() {
        ...
        if (!$this->ion_auth->logged_in()) {
            ...
            $user_type = $this->ion_auth->user_group_name();
            if ($user_type=='admin') {
                $this->load->library('pagination');
                $config = $this->config->item('pagination');
                $config["base_url"] = site_url('auth/index');
                $config["total_rows"] = $this->ion_auth->getSalesPersonsCount();
                $this->pagination->initialize($config);
                $page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
                $this->data["links"] = $this->pagination->create_links();
                $this->data['title'] = $this->lang->line('index_heading');
                $this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
                $this->data['users'] = $this->ion_auth->getSalesPersons($config['per_page'], $page);
                foreach ($this->data['users'] as $k => $user) {
                    $this->data['users'][$k]->groups = $this->ion_auth->get_users_groups($user->id)->result();
                }
                $this->_render_page('auth' . DIRECTORY_SEPARATOR . 'index', $this->data);
            }else if($user_type=='client') {
                redirect("clients/home", 'refresh');
            }else{
...
//		}
    ...
    public function delete_user($id) {
        ...
        if ($this->ion_auth->delete_user($id)) {
            ...
            $this->Ion_auth_model->updateClientsCreatedBy($id, $user->id);
            $this->session->set_flashdata('msg', 'Salesman deleted.');
            redirect('auth', 'refresh');
        } else {
            $this->session->set_flashdata('msg', 'Unable to delete Salesman, Please try again.');
            redirect('auth', 'refresh');
        }
    }
