...
class Ion_auth
{
	...
	public function user_group_name($id = FALSE) {//returns user group title admin/client/member etc
        $user_group = $this->get_users_groups($id)->row();
        return $user_group->name ;
    }

	public function get_client_permissions(){
		return $this->ion_auth_model->get_client_permissions($this->session->userdata('user_id'));
	}
