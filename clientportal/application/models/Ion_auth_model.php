...
class Ion_auth_model extends CI_Model
{
	...
	public function getUserWithAgreement ($user_id = null) {
		$this->db->select('users.id, users.email, users.first_name, users.last_name, users.active, clients.status, clients.id as ag_id');
		$this->db->from('users');
		$this->db->join('clients', 'clients.client_id = users.id', 'left');
		if ($user_id) {
	...
	public function get_client_permissions($id){
        return $this->db->where(['user_id'=>$id])->get('client_permission')->result_array();
    }

	public function getSalesPersons ($limit, $offset) {
		$this->db->select('users.id, users.email, users.first_name, users.last_name, users.active');
		$this->db->from('users');
	...
	public function getSalesPersonsCount () {
		$this->db->select('users.id, users.email, users.first_name, users.last_name, users.active');
		$this->db->from('users');
		$this->db->join('users_groups', 'users_groups.user_id = users.id AND users_groups.group_id = 3');
    ...
    public function updateClientsCreatedBy ($salesman_id, $admin_id) {
        $this->db->update('users', ['created_by' => $admin_id], ['created_by' => $salesman_id]);
        $this->db->update('agreements', ['created_by' => $admin_id], ['created_by' => $salesman_id]);
    }
